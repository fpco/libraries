...
runPureStatefulSlave :: forall m context input state output a.
       (MonadConnect m, NFData state, NFData output, Store state, NFData input)
    => (context -> input -> state -> m (state, output))
...
-- and receiving data.
runNMStatefulSlave ::
       (MonadConnect m, NFData state, Store state, NFData output, Store output, Store context, Store input, NFData input)
...
runRequestedStatefulSlave ::
       (MonadConnect m, NFData state, Sendable state, NFData output, Sendable output, Sendable context, Sendable input, NFData input)
    => Redis
    -> Milliseconds
    -> Update m state context input output
    -> m void
...
runJobQueueStatefulWorker ::
       (MonadConnect m, NFData state, Sendable state, NFData output, Sendable output, Sendable context, Sendable input, Sendable request, Sendable response, NFData input)
    => JobQueueConfig
    -> CN.ServerSettings
    -- ^ The settings used to create the server. You can use 0 as port number to have
    -- it automatically assigned
