...
module Distributed.JobQueueSpec2 (spec) where
...
chaosTest = do
    -- We send 1000 requests, with 100 workers and 1000 clients.
    -- In the first phase the workers and are started killed at a rate that
    ...
    fmap (either absurd id) (Async.race (runChaosWorkers workStartedCountRef workCompletedCountRef) runChaosClients)
  where
    numClients :: Int
    numClients = 100
    ...
    runChaosWorkers workStartedCountRef workCompletedCountRef = do
        ...
        let runChaosWorker :: Int -> m void
            runChaosWorker _ = do
                replicateM_ 100 $
                    maybe () absurd <$> timeout phaseOneLifespan testJobWorker
                killRandomly phaseTwoKillRandomly jw
        absurd . NE.head <$> Async.mapConcurrently runChaosWorker (NE.fromList [1..numWorkers])
